<?php
$folderPath = realpath(__DIR__ . '/../files/2026 revision/Past paper');
$files = [];
    ...
    <div id="menubar">
     ...
     <div class="note" style="text-align:center; margin-top: 200px;">
        <?php if ($folderNotFound): ?>
            <h3 style="color:red;">⚠️ Folder "<?= htmlspecialchars('../files/2026 revision/Past paper') ?>" does not exist.</h3>
        <?php elseif (empty($files)): ?>
            <h3 style="color:red;">⚠️ No files found in the Past Paper folder.</h3>
        <?php else: ?>
            <?php foreach ($files as $file): ?>
                <button>
                    <a href="<?= '../files/2026 revision/Past paper/' . rawurlencode($file) ?>" target="_blank">
