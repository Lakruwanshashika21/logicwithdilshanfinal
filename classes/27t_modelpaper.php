<?php
$folderPath = realpath(__DIR__ . '/../files/2027 theory/Model paper');
$files = [];
...
<body>
    ...
    <div class="note" style="text-align:center; margin-top: 200px;">
        <?php if ($folderNotFound): ?>
            <h3 style="color:red;">⚠️ Folder "<?= htmlspecialchars('../files/2027 theory/Model paper') ?>" does not exist.</h3>
        <?php elseif (empty($files)): ?>
            <h3 style="color:red;">⚠️ No files found in the Model paper folder.</h3>
        <?php else: ?>
            <?php foreach ($files as $file): ?>
                <button>
                    <a href="<?= '../files/2027 theory/Model paper/' . rawurlencode($file) ?>" target="_blank">
