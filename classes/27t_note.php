<?php
$folderPath = realpath(__DIR__ . '/../files/2027 theory/Tute');
$files = [];
    ...
    <div id="menubar">
     ...
     <div class="note" style="text-align:center; margin-top: 200px;">
        <?php if ($folderNotFound): ?>
            <h3 style="color:red;">⚠️ Folder "<?= htmlspecialchars('../files/2027 theory/Tute') ?>" does not exist.</h3>
        <?php elseif (empty($files)): ?>
            <h3 style="color:red;">⚠️ No files found in the Tute folder.</h3>
        <?php else: ?>
            <?php foreach ($files as $file): ?>
                <button>
                    <a href="<?= '../files/2027 theory/Tute/' . rawurlencode($file) ?>" target="_blank">
