<?php
$folderPath = realpath(__DIR__ . '/../files/2025 theory/MIT');  // absolute path for security & correctness
...
<body>
    ...
    <div class="note" style="text-align:center; margin-top: 200px;">
        <?php if ($folderNotFound): ?>
            <h3 style="color:red;">⚠️ Folder "<?= htmlspecialchars('../files/2025 theory/MIT') ?>" does not exist.</h3>
        <?php elseif (empty($files)): ?>
            <h3 style="color:red;">⚠️ No files found in the MIT folder.</h3>
        <?php else: ?>
            <?php foreach ($files as $file): ?>
                <button>
                    <a href="<?= '../files/2025 theory/MIT/' . rawurlencode($file) ?>" target="_blank">
