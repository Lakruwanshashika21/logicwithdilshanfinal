<?php
$folderPath = realpath(__DIR__ . '/../files/2026 theory/Past paper');
$files = [];
...
<body>
  ...
  <div class="note" style="text-align:center; margin-top: 200px;">
    <?php if ($folderNotFound): ?>
      <h3 style="color:red;">⚠️ Folder "<?= htmlspecialchars('../files/2026 theory/Past paper') ?>" does not exist.</h3>
    <?php elseif (empty($files)): ?>
      <h3 style="color:red;">⚠️ No files found in the Past paper folder.</h3>
    <?php else: ?>
      <?php foreach ($files as $file): ?>
        <button>
          <a href="<?= '../files/2026 theory/Past paper/' . rawurlencode($file) ?>" target="_blank">
